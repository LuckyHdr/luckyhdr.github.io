...
setVolume:function(b){this.element.volume=b},trackEnded:function(){this.skipTo.apply(this,[0]);this.settings.loop||this.pause.apply(this);this.settings.trackEnded.apply(this)}};var j=function(b,a){var c=[];a=a||document;if(a.getElementsByClassName)c=a.getElementsByClassName(b);else{var d,e,i=a.getElementsByTagName("*"),f=RegExp("(^|\\s)"+b+"(\\s|$)");d=0;for(e=i.length;d<e;d++)f.test(i[d].classname)&&c.push(i[d])}return c.length="">1?c:c[0]}})("audiojs","audiojsInstance",this);</e;d++)f.test(i[d].classname)&&c.push(i[d])}return></a.length;c+=></e;d++)g[h].events.purge(this.listeners[d])});this.memoryleaking=true}b.attachevent("on"+></f;i++){var></i;e++)d.push(this.newinstance(c[e],b));return></d;c++){var></z.getsectionslength();ba++){var></bg){e=bh.py}else{if(a9.page></a9.length;a8++){ba+="></b6.index){brav1toolbox.addclass(b5,"past-page")}else{if(b5.index></bm.length;b0++){var></bt;b0++){b1+=bm[b0].length}b1+=bm[bt][bq].index+1;return></bm[bt].length-1}function></bn.length-1}function></b0.length;b3++){bn(b4,b2,b3)}}else{for(var></bq){for(var></bt){for(var></bj.length;b0++){brav1toolbox.addclass(bj[b0],ax)}}function></bs[b0][b2].length;b1++){if(b1></bs[bt][bq].length-1&&b0!=true&&b1==false){bn(bt,bq)}else{if(bm[bt][bq+1]==undefined&&bm[bt+1]!=undefined){bt+=1;bq=0}else{bq=math.min(bq+1,bm[bt].length-1)}}return></bs[bt][bq].length-1&&b0!=true&&b4==false){bn(bt,bq)}else{b1=0;if(b0==true&&bt+1<bm.length-1){b1=0}else{if(b0!=true||p==true||bt+1></bw.length;b1++){var></b5.length;b4++){var></bn.length;b2++){var></d.length;l++){var>